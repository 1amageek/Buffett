...
struct ChartViewModelTests {
    ...
    @Test func testUpdateIndicatorsWithEmptyData() {
        ...
        #expect(viewModel.vwap.isEmpty)
        #expect(viewModel.priceAnnotations.isEmpty) // Also check annotations are empty
    }

    // MARK: - Price Annotation Tests

    @Test func testAddPriceAnnotation() {
        let mockAPI = MockStockAPI()
        let viewModel = ChartViewModel(symbol: SampleData.symbol, api: mockAPI)

        #expect(viewModel.priceAnnotations.isEmpty)

        // Add first annotation
        viewModel.addPriceAnnotation(at: 150.0, label: "Support 1")
        #expect(viewModel.priceAnnotations.count == 1)
        let firstAnnotation = viewModel.priceAnnotations.first
        #expect(firstAnnotation?.priceLevel == 150.0)
        #expect(firstAnnotation?.label == "Support 1")
        #expect(firstAnnotation?.color == .orange) // Default color

        // Add second annotation with a specific color
        viewModel.addPriceAnnotation(at: 160.0, label: "Resistance 1", color: .blue)
        #expect(viewModel.priceAnnotations.count == 2)
        let secondAnnotation = viewModel.priceAnnotations.last
        #expect(secondAnnotation?.priceLevel == 160.0)
        #expect(secondAnnotation?.label == "Resistance 1")
        #expect(secondAnnotation?.color == .blue)
    }

    @Test func testRemovePriceAnnotationByID() {
        let mockAPI = MockStockAPI()
        let viewModel = ChartViewModel(symbol: SampleData.symbol, api: mockAPI)

        viewModel.addPriceAnnotation(at: 150.0, label: "Support")
        viewModel.addPriceAnnotation(at: 160.0, label: "Resistance")
        
        #expect(viewModel.priceAnnotations.count == 2)
        guard let firstAnnotationID = viewModel.priceAnnotations.first?.id else {
            Issue.record("Failed to get first annotation ID")
            return
        }
        let nonExistentID = UUID()

        // Remove existing annotation by ID
        viewModel.removePriceAnnotation(id: firstAnnotationID)
        #expect(viewModel.priceAnnotations.count == 1)
        #expect(viewModel.priceAnnotations.contains(where: { $0.id == firstAnnotationID }) == false)
        #expect(viewModel.priceAnnotations.first?.priceLevel == 160.0) // The second one should remain

        // Attempt to remove non-existent ID
        viewModel.removePriceAnnotation(id: nonExistentID)
        #expect(viewModel.priceAnnotations.count == 1) // Count should not change
    }

    @Test func testRemovePriceAnnotationByObject() {
        let mockAPI = MockStockAPI()
        let viewModel = ChartViewModel(symbol: SampleData.symbol, api: mockAPI)

        viewModel.addPriceAnnotation(at: 150.0, label: "To Remove")
        viewModel.addPriceAnnotation(at: 160.0, label: "To Keep")
        
        #expect(viewModel.priceAnnotations.count == 2)
        
        guard let annotationToRemove = viewModel.priceAnnotations.first(where: { $0.label == "To Remove" }) else {
            Issue.record("Failed to find annotation 'To Remove'")
            return
        }
        let annotationNotInList = PriceAnnotation(priceLevel: 170.0, label: "Not Added")

        // Remove existing annotation by object
        viewModel.removePriceAnnotation(annotation: annotationToRemove)
        #expect(viewModel.priceAnnotations.count == 1)
        #expect(viewModel.priceAnnotations.contains(where: { $0.id == annotationToRemove.id }) == false)
        #expect(viewModel.priceAnnotations.first?.label == "To Keep")

        // Attempt to remove an object not in the list
        viewModel.removePriceAnnotation(annotation: annotationNotInList)
        #expect(viewModel.priceAnnotations.count == 1) // Count should not change
    }

    @Test func testAddMultipleAnnotationsAndRemoveAll() {
        let mockAPI = MockStockAPI()
        let viewModel = ChartViewModel(symbol: SampleData.symbol, api: mockAPI)

        let annotation1Price = 150.0
        let annotation2Price = 160.0
        let annotation3Price = 170.0

        viewModel.addPriceAnnotation(at: annotation1Price, label: "L1")
        viewModel.addPriceAnnotation(at: annotation2Price, label: "L2")
        viewModel.addPriceAnnotation(at: annotation3Price, label: "L3")

        #expect(viewModel.priceAnnotations.count == 3)

        guard let id1 = viewModel.priceAnnotations.first(where: {$0.priceLevel == annotation1Price})?.id else {
            Issue.record("Missing ID1"); return
        }
        guard let id2 = viewModel.priceAnnotations.first(where: {$0.priceLevel == annotation2Price})?.id else {
            Issue.record("Missing ID2"); return
        }
        guard let id3 = viewModel.priceAnnotations.first(where: {$0.priceLevel == annotation3Price})?.id else {
            Issue.record("Missing ID3"); return
        }

        viewModel.removePriceAnnotation(id: id1)
        #expect(viewModel.priceAnnotations.count == 2)
        #expect(viewModel.priceAnnotations.contains(where: { $0.id == id1 }) == false)

        viewModel.removePriceAnnotation(id: id2)
        #expect(viewModel.priceAnnotations.count == 1)
        #expect(viewModel.priceAnnotations.contains(where: { $0.id == id2 }) == false)

        viewModel.removePriceAnnotation(id: id3)
        #expect(viewModel.priceAnnotations.isEmpty)
        #expect(viewModel.priceAnnotations.contains(where: { $0.id == id3 }) == false)
    }
}
