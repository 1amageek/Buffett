    ...
    public var body: some View {
        VStack(spacing: 0) {
            ...
            if viewModel.data.isEmpty && viewModel.allFetchedData.isEmpty {
                ...
                SymbolChartView(
                    ...
                    vwap: viewModel.vwap,
                    annotations: viewModel.priceAnnotations // Pass annotations
                )
            }
        }
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                Button("Add Test Annotation") {
                    let price = viewModel.data.last?.close ?? viewModel.allFetchedData.last?.close ?? 100.0 // Use last known close or a default
                    let label = String(format: "Test @ %.2f", price)
                    viewModel.addPriceAnnotation(at: price, label: label)
                }
                .disabled(viewModel.allFetchedData.isEmpty && viewModel.data.isEmpty) // Disable if no data to base price on
            }
        }
        .task {
...
#Preview("Default (1 Year) with Annotations") {
    let symbol = Symbol(code: "AAPL", name: "Apple Inc.")
    let mockAPI = SymbolChartScreenPreviewMockStockAPI() // Uses default ~3 months data
    let viewModel = ChartViewModel(symbol: symbol, api: mockAPI)
    // ViewModel's selectedPeriod defaults to .oneYear, so it will show all 3 months of mock data
    
    // Add initial annotations for preview
    // Ensure these prices make sense for the default mock data (which is around 130-170)
    viewModel.priceAnnotations = [
        PriceAnnotation(priceLevel: 150.0, label: "Initial Support"),
        PriceAnnotation(priceLevel: 165.0, color: .blue, label: "Initial Resistance")
    ]
